...
func (in *EC2NodeClass) InstanceProfileName(clusterName, region string) string {
	version := 0
	if in.Status.InstanceProfiles != nil {
		version = in.Status.InstanceProfiles.Version
	}
	return fmt.Sprintf("%s_%d_%d", clusterName, lo.Must(hashstructure.Hash(fmt.Sprintf("%s%s", region, in.Name), hashstructure.FormatV2, nil)), version)
}
